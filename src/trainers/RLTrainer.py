...
class RLTrainer():
    ...
    def __init__(self, max_episodes=1e6,
    ...
    #     # Batch of ground truth images and corresponding masked images
    #     # Index 0 is complete image and index 1 is masked image
    #     masked_input_batch = self.get_train_batch()[1]
    ...
    #     # In the in initial exploration phase we run pure random uniform policy
    #     if self.current_episode < self.initial_exploration_episodes:
    #         action = torch.FloatTensor(masked_input_batch.shape[0], self.action_dim).uniform_(-self.DDPG.action_value_range, self.DDPG.action_value_range).to(input_batch.device)
    #     else:
    #         action = self.DDPG.select_action(current_state)
    #         if self.exploration_noise != 0:
    #             action = torch.clamp((action + torch.randn(action.shape, device=action.device)* self.exploration_noise),
    #                                  min=torch.ones_like(action, device=action.device) * -self.DDPG.action_value_range,
    #                                  max=torch.ones_like(action, device=action.device) * self.DDPG.action_value_range)
    ...
    #     imgs = []
    #     for i in range(10):
    #         # Batch of masked images
    #         input_batch = self.get_val_batch()[1]
            
    #         # Convert the masked image to GFV
    #         current_state = self.env.get_state(input_batch)
    #         done = False
