...
class Generator(nn.Module):
    ...
    def __init__(self, z_dim=16, GFV_dim=1024, image_size=64, conv_dim=64):
...
#     def __init__(self, z_dim=16, gfv_dim=2048, conv_dim=64):
#         super(Generator, self).__init__()
#         self.gfv_dim = gfv_dim
...
#         out, p1 = self.attn1(out)  # (batch_size, 512, 16, 16)
#         out = self.l4(out)  # (batch_size, conv_dim * mult / 8, 32, 32)
#         out, p2 = self.attn2(out)  # (batch_size, 256, 32, 32)
#         out = self.last(out)  # (batch_size, gfv_dim, 64, 64)
#         out = F.adaptive_avg_pool2d(out, (1, 1))  # (batch_size, gfv_dim, 1, 1)
#         return out.view(out.size(0), -1) # (batch_size, gfv_dim)


# class Discriminator(nn.Module):
#     """Discriminator."""

#     def __init__(self, gfv_dim=2048, input_dim=64, conv_dim=128):
#         super(Discriminator, self).__init__()
#         repeat_num = int(np.log2(gfv_dim // 64))  # Calculate repeat_num based on gfv_dim

#         layer0 = nn.ConvTranspose1d(gfv_dim, gfv_dim*2 ,1)
        
#         curr_dim = conv_dim
#         layer1 = [
#             spectral_norm(nn.Conv2d(input_dim, curr_dim, 3, 1, 1)),
#             nn.LeakyReLU(0.1)
#         ]
...
#         curr_dim = curr_dim * 2
#         layer2 = [
#             spectral_norm(nn.Conv2d(input_dim, curr_dim, 3, 2, 1)),
#             nn.LeakyReLU(0.1)
#         ]
...
#         self.multihead_attn = nn.MultiheadAttention(embed_dim=in_dim, num_heads=num_heads)
#         self.gamma = nn.Parameter(torch.zeros(1))
...
#         out = self.gamma * attn_output + x.squeeze(1)
#         return out
